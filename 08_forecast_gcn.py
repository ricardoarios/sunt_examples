...
------------
  ...
  GCN xk : aggregate from graph neighbours   → (B, N, GCN_HIDDEN)
...
if PYG_AVAILABLE:
    ...
    model = GCNForecasterPyG(INPUT_LEN, GCN_HIDDEN, HORIZON, NUM_LAYERS, DROPOUT).to(
        DEVICE
    )
else:
    model = GCNForecaster(INPUT_LEN, GCN_HIDDEN, HORIZON, NUM_LAYERS, DROPOUT).to(
        DEVICE
    )
...
for epoch in range(1, EPOCHS + 1):
    ...
    for xb, yb in train_loader:
        ...
        optimizer.zero_grad()
        pred = (
            model(xb, edge_index, edge_weight)
            if PYG_AVAILABLE
            else model(xb, adj_tensor)
        )
...
ax.set_title(
    f"GCN | Stop {stop_name} | MAE={mae:.1f}  RMSE={rmse:.1f}  MAPE={mape:.1f}%",
    fontsize=11,
)
...
fig.tight_layout()
fig.savefig(OUTPUT_DIR / "gcn_forecast.png", dpi=150)
...
fig.tight_layout()
fig.savefig(OUTPUT_DIR / "gcn_per_node_mae.png", dpi=150)
