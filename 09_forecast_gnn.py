...
class TGCN(nn.Module):
    ...
    def __init__(
        ...
        self.gcn_layers = nn.ModuleList(
            [
                GraphConvolution(in_features if i == 0 else gcn_hidden, gcn_hidden)
...
ax.set_xlabel("Epoch")
ax.set_ylabel("MSE")
ax.set_title("T-GCN – training curves")
ax.legend()
...
ax.plot(
    ...
    lw=1.5,
    linestyle="--",
    label="T-GCN forecast",
)
ax.set_title(
    f"T-GCN | Stop {stop_name} | MAE={mae:.1f}  RMSE={rmse:.1f}  MAPE={mape:.1f}%",
    fontsize=11,
)
...
fig.tight_layout()
fig.savefig(OUTPUT_DIR / "tgcn_forecast.png", dpi=150)
...
fig.tight_layout()
fig.savefig(OUTPUT_DIR / "tgcn_per_node_mae.png", dpi=150)
