...
print(
    f"Series length: {len(series)} hours  ({series.index[0].date()} → {series.index[-1].date()})"
)
...
print(
    f"KPSS test – stat={kpss_stat:.4f}, p={kpss_p:.4f}  "
...
plot_acf(
    train, lags=max_lags, ax=axes[0], title=f"ACF – stop {STOP_ID} (hourly boardings)"
)
