...
Architecture:
  Positional Encoding
  → N x TransformerEncoderLayer (self-attention over time)
  → Global-average pooling across time
  → Linear head (forecast horizon x n_features)
...
class TransformerForecaster(nn.Module):
    def __init__(
        self, n_features, d_model, n_heads, n_layers, ffn_dim, horizon, dropout
    ):
        super().__init__()
...
ax.set_title(
    f"Transformer | Stop {stop_name} | MAE={mae:.1f}  RMSE={rmse:.1f}  MAPE={mape:.1f}%",
    fontsize=11,
)
